...
data class QuizHistory(
    ...
    companion object {
        fun fromQuizResultEntity(
            quizResultEntity: com.neo.trivia.data.database.entity.QuizResultEntity
        ): QuizHistory {
            return QuizHistory(
                id = quizResultEntity.id,
                categoryName = quizResultEntity.categoryName,
                quizDate = java.text.SimpleDateFormat("MMM dd, yyyy", java.util.Locale.getDefault())
                ...
                accuracy = if (quizResultEntity.totalQuestions > 0)
